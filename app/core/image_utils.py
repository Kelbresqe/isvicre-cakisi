...
async def load_and_validate_image(
    ...
    if file:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                raise HTTPException(
                    status_code=400, detail=f"URL indirilemedi: {str(e)}"
                )
    else:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Resim yüklenirken hata oluştu: {str(e)}"
        )


def save_image(
    img: Image.Image, filename: str, target_format: str, **save_kwargs
) -> tuple[Path, str, int]:
    """
    ...
    try:
        ...
        output_path = (
            settings.TEMP_DIR / f"processed_{os.urandom(4).hex()}_{output_filename}"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Resim kaydedilirken hata oluştu: {str(e)}"
        )
